...
<div class="wrapper wrapper-content animated fadeInRight">
    ...
    <a href="{!!url('qlist')!!}/{{$question->id}}" class = 'btn btn-primary'><i class="fa fa-home"></i>Буцах</a>
    <br>
    <br/>
    <form method = 'POST' action = '{!! url("questionlist")!!}/{!!$question->
        id!!}/update'> 
        <input type = 'hidden' name = '_token' value = '{{Session::token()}}'>
        <div class="form-group">
            <label for="question">Асуулт</label>
            <input id="question" name = "question" type="text" class="form-control" value="{!!$question->
            questionname!!}">
        </div>
        <div class="form-group">
            <label for="description">Тайлбар</label>
            <input id="description" name = "description" type="text" class="form-control" value="{!!$question->
            desc!!}">
        </div>
        <div class="form-group">
            ...
            <select name="type" class="form-control">
                <option @if($question->type==0) selected @endif value="0">Зөвхөн нэгийг сонгох</option>
                <option @if($question->type==1) selected @endif value="1">Олныг сонгох боломжтой</option>
            </select>
        </div>
        <div class="panel panel-warning">
            ...
            <div class="panel-body" id="mbanswersparent">
                ...
                <div style="border: 1px #cccccc dotted; padding: 30px">
                    ...
                    <textarea id="answers" name="answers" type="text" class="form-control">{{unserialize($question->answers)}}</textarea>
                </div>
            </div>
        </div>
...
@section('script')
    <script>
        var answers = JSON.parse($('#answers').val());// [];//[{name:'Хариулт 1',score:1},{name:'Хариулт 2',score:0}];
        console.log(answers);
        function load() {
            $('#mbanswers').html('');
            if (answers.length == 0) {
